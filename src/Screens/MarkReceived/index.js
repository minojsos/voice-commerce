...
const MainScreen = ({routes, route, navigation}) => {
  ...
  useEffect(() => {
    ...
    Tts.speak(translations.formatString(translations['markOrderReceivedTts'], {order_id: 1}), 
      { 
        androidParams: {
          KEY_PARAM_PAN: -1,
      ...
      if (menuitem.includes("cancel")) {
        ...
        if (list.length > 1 && list[1] != "") {
          ...
          fetch(`${BASE_URL}/completed`, {
            ...
            .then((response) => {
              Tts.speak(translations['successReceived'])
        ...
        <ScrollView>
          <View
            style={{
              ...
              { order != null ?
              <>
                <Card accessible={true} accessibilityLabel={translations.formatString(translations['viewOrderText'], {order_id: order.order_id})} accessibilityHint={translations.formatString(translations['reviewOrderTextHint'], {order_id: order.order_id})}>
                  <Card.Content>
                    <Title>{translations.formatString(translations['reviewOrderText'], {order_id: order.order_id})}</Title>
                    <InputX
                      multiline={true}
                      numberOfLines={4}
                      accessible={true}
                      label={translations['reviewLabel']}
                      accessibilityLabel={translations['reviewLabel']}
                      accessibilityHint={translations.formatString(translations['reviewHint'], {order_id: order.order_id})}
                  ...
                  <ButtonX
                      ...
                      accessibilityHint={translations.formatString(translations['confirmReviewHint'])}
