...
const MainScreen = ({routes, navigation}) => {
  ...
  const createData = () => {
    ...
    orders.push({"order_id":1,"shop_id":1,"shop_name":"Taniya","user_id":1,"coupon_id":1,"coupon_code":"CV100", "coupon_value":100, "order_status":0,"order_payment":0,"items":
                                                                                              [{"item_id":1,"item_name":"Rice","item_code":"Rice001","item_rate":200,"item_offer_price":null,"item_qty":1},
  ...
  const getData = async () => {
    try {
      ...
      for (var i=0; i<orders.length; i++) {
        if (orders[i].order_status == 2 || orders[i].order_status == 3) {
      ...
      setCancelledOrdersList(allOrders)
    } catch (e) {
      console.log('ee');
    }
  }
  ...
  useEffect(() => {
    ...
    for (var i = 0; i < cancelledOrdersList.length; i++) {
      var itms = cancelledOrdersList[i].items;
      var total=0;
      for (var j=0; j < itms.length; j++) {
        if (itms[j].item_offer_price != null) {
          total += itms[j].item_offer_price;
        } else {
          total += itms[j].item_rate;
        }
      }

      msg+=translations.formatString(translations['orderDetailInfo'], {order_id: cancelledOrdersList[i].order_id, shop_name: cancelledOrdersList[i].shop_name, total: total, length: itms.length})
    }
      ...
      if (menuitem.includes("order")) {
        ...
        for(var i =0; i < list.length; i++) {
          if (!isNaN(list[i])) {
            ...
            for (var i=0; i < cancelledOrdersList.length; i++) {
              if (cancelledOrdersList[i].order_id == list[i]) {
                // matching order

                navigation.navigate('orderCancelled', cancelledOrdersList[i])
              }
            }
          }
        }
  ...
  const viewOrder = function (id) {
    ...
    navigation.navigate('orderCancelled', id)
  }
            ...
            style={{
            ...
            }}>
              {
                ...
                return (
                  ...
                  onLongPress={() => navigation.navigate('orderCancelled', order.order_id)}
                  key={order.order_id}
                  >
                    <Card.Content>
