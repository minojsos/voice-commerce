...
const MainScreen = ({routes, route, navigation}) => {
  ...
  useEffect(() => {
    ...
    Tts.speak(translations.formatString(translations['markOrderReturnedtts'], {order_id: 1}), 
      { 
        androidParams: {
          KEY_PARAM_PAN: -1,
      ...
      if (menuitem.includes("cancel")) {
        ...
        if (list.length > 1 && list[1] != "") {
          ...
          fetch(`${BASE_URL}/return`, {
            ...
            .then((response) => {
              Tts.speak(translations['successReturn'])
        ...
        <ScrollView>
          <View
            style={{
              ...
              { order != null ?
              <>
                <Card accessible={true} accessibilityLabel={translations.formatString(translations['viewOrderText'], {order_id: order.order_id})} accessibilityHint={translations.formatString(translations['returnOrderTextHint'])}>
                  <Card.Content>
                    <Title>{translations.formatString(translations['returnOrderText'], {order_id: order.order_id})}</Title>
                    <InputX
                      multiline={true}
                      numberOfLines={4}
                      accessible={true}
                      label={translations['returnLabel']}
                      accessibilityLabel={translations['returnLabel']}
                      accessibilityHint={translations.formatString(translations['returnHint'], {order_id: order.order_id})}
                  ...
                  <ButtonX
                      ...
                      accessibilityHint={translations.formatString(translations['confirmReturnHint'], {order_id: order.order_id})}
